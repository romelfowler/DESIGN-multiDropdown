document.addEventListener("DOMContentLoaded", function(){
    ...
    function updateSelectedOptions(customSelect){
        const selectedOptions = Array.from(customSelect.querySelectorAll(".option.active")).filter(option => 
            option !== customSelect.querySelectorAll(".option.all-tags")).map(
                function(option){
                    return {
                ...
                if(selectedOptions.length === 0 ) {
                    tagsHTML = '<span class="placeholder">Select the program</span>'
                }else {
    ...
    customSelects.forEach(function(customSelect){
        ...
        allTagsOption.addEventListener("click", function(option){
            if(option !== allTagsOption){
                option.classList.toggle("active", !isAcitve);
            }
        });
